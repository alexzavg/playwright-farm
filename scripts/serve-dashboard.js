...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .step-group.open .step-header { border-bottom: 1px solid #475569; }
    
    .error-group { border-top: 1px solid #334155; }
    .error-header {
    ...
    .error-count {
      ...
      border-radius: 10px; font-size: 12px; margin-left: 10px; flex-shrink: 0;
    }
    
    .traces-list { display: none; padding: 10px 20px 15px 60px; }
    ...
    .trace-info { font-size: 13px; color: #94a3b8; }
    .trace-link {
...
function generateFailuresHTML(failures) {
  const steps = Object.keys(failures);
  ...
  return steps.map(stepName => {
    const errors = failures[stepName];
    ...
    const errorsHTML = Object.keys(errors).map(errorMsg => {
      ...
      const tracesHTML = errorData.traces.map(t => {
        ...
        <div class="trace-item">
          <span class="trace-info">${escapeHtml(t.timestamp)} - ${escapeHtml(t.title)}</span>
          <a class="trace-link" href="${tracePath}" target="_blank">📋 View Trace</a>
        </div>
      `;
      }).join('');
    ...
    return `
      <div class="step-group">
        <div class="step-header">
          ...
          <span class="step-count">${totalForStep} failures</span>
        </div>
